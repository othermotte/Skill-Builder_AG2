...
import { User, Role, Scenario, Skill, PracticeSession, PracticeAttempt } from '../types';
...
export const DashboardPage: React.FC<DashboardPageProps> = ({ 
  currentUser, scenarios, skills, practiceSessions, practiceAttempts,
  onStartPractice,
  onUserUpdate, 
  onViewHistory,
  ...
  // Calculate Scenario Depth Map
  const scenarioDepthMap = useMemo(() => {
    const map: Record<string, number> = {};
    practiceAttempts.forEach(attempt => {
      if (attempt.completedAt) {
        map[attempt.scenarioId] = (map[attempt.scenarioId] || 0) + 1;
      }
    });
    return map;
  }, [practiceAttempts]);
    ...
    <div className="space-y-16 pb-20 max-w-5xl mx-auto">
        ...
        <section className="px-4 border-t border-gray-100 pt-16">
          ...
          <div className="space-y-4">
            {practiceSessions.slice(0, 3).map(session => (
              <div key={session.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl border border-gray-100">
      ...
      <section className="pt-12 border-t border-gray-100 text-center px-4">
          {!showFeedbackForm ? (
            ...
            >
              How can we make the Studio better? Leave feedback.
            </button>
          ) : (
